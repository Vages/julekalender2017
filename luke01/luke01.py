...
def character_set_of_word(word: str) -> CharSet:
    return set(word)


def character_count_of_word(word: str) -> CharCount:
    return Counter(word)
...
def find_words_that_have_same_charset_as_riddle(riddle: str, word_list: WordList) -> WordList:
    riddle_charset: CharSet = character_set_of_word(riddle)
    return list(filter(lambda x: character_sets_are_the_same(riddle_charset, character_set_of_word(x)), word_list))
...
def word_has_matching_n_gram_string(riddle: str, word: str) -> typing.Tuple[bool, int]:
    ...
    riddle_charcount: CharCount = character_count_of_word(riddle)
    for n, word in enumerate(n_grams, start=1):
        if character_counts_are_the_same(riddle_charcount, character_count_of_word(word)):
            return True, n
